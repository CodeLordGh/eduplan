...
import { createForbiddenError, throwError } from '../errors';
import type {
  UserAttributes,
  AccessPolicy,
  ValidationResult,
...
const validateRoles = (
  ...
  if (conditions.anyOf?.roles) {
    ...
    if (!hasAnyRole) {
      return E.left(
        createRoleError('User does not have any of the required roles', {
  ...
  if (conditions.allOf?.roles) {
    ...
    if (!hasAllRoles) {
      return E.left(
        createRoleError('User does not have all required roles', {
...
const validateVerification = (
  ...
  if (conditions.verification?.requireKYC && user.kyc.status !== 'VERIFIED') {
    return E.left(
      createVerificationError('KYC verification required', {
  ...
  if (
    conditions.verification?.kycStatus &&
  ...
  if (
    conditions.verification?.employmentStatus &&
  ...
  if (conditions.verification?.officerPermissions) {
    const hasPermissions = conditions.verification.officerPermissions.every(
      (permission: string) =>
        user.kyc.officerStatus?.permissions &&
        permission in user.kyc.officerStatus.permissions &&
        user.kyc.officerStatus.permissions[
          permission as keyof typeof user.kyc.officerStatus.permissions
        ]
    );
    if (!hasPermissions) {
      return E.left(
...
const validateSchoolContext = (
  ...
  if (mustBeInSchool && Object.keys(user.schoolRoles).length === 0) {
    return E.left(
      createSchoolContextError('User must be associated with a school', {
        userId: user.id,
  ...
  if (mustBeCurrentSchool && !user.context.currentSchoolId) {
    return E.left(
      createSchoolContextError('No current school context', {
        userId: user.id,
  ...
  if (mustBeOwner) {
    ...
    if (!isOwner) {
      return E.left(
        createSchoolContextError('User must be a school owner', {
          userId: user.id,
          schoolRoles: user.schoolRoles,
        })
      );
...
const validateEnvironment = (
  ...
  const { ipRestrictions, timeRestrictions, deviceRestrictions, locationRestrictions } =
  ...
  if (ipRestrictions) {
    ...
    if (!userIp)
      return E.left(
        createEnvironmentError('IP address not available', {
          userId: user.id,
          context: user.context,
        })
      );
    ...
    if (ipRestrictions.allowlist && !ipRestrictions.allowlist.includes(userIp)) {
      return E.left(
        createEnvironmentError(`IP ${userIp} not in allowlist`, {
          userIp,
    ...
    if (ipRestrictions.denylist && ipRestrictions.denylist.includes(userIp)) {
      return E.left(
        createEnvironmentError(`IP ${userIp} is in denylist`, {
          userIp,
  ...
  if (deviceRestrictions) {
    ...
    if (!userDevice)
      return E.left(
        createEnvironmentError('Device information not available', {
          userId: user.id,
          context: user.context,
        })
      );
  ...
  if (locationRestrictions) {
    ...
    if (!userLocation)
      return E.left(
        createEnvironmentError('Location information not available', {
          userId: user.id,
          context: user.context,
        })
      );
    ...
    if (
      ...
      return E.left(
        createEnvironmentError(`Access not allowed from country: ${userLocation.country}`, {
    ...
    if (
      ...
      return E.left(
        createEnvironmentError(`Access not allowed from region: ${userLocation.region}`, {
...
const validateCustomConditions = (
  ...
  for (const condition of conditions.custom) {
    if (!condition.evaluator(user, context)) {
      return E.left(
        createForbiddenError(condition.errorMessage, {
...
export const createAbacMiddleware =
  ...
  async (req: any, res: any, next: any): Promise<void> => {
    try {
      const validationResult = validateAccess(req.userAttributes, policy, {
        params: req.params,
        query: req.query,
        body: req.body,
      });
      ...
      if (!validationResult.granted) {
        throwError(
          createForbiddenError(validationResult.reason || 'Access denied', {
      ...
      if (error instanceof Error) {
        next(
          createForbiddenError('Access check failed', {
            code: 'ACCESS_CHECK_ERROR',
            resource: policy.resource,
            action: policy.action,
            userId: req.userAttributes?.id,
