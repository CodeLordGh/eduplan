...
export const getRedisValue = (redis: Redis) => (key: string): TaskEither<AppError, Option<string>> =>
  tryCatch(
    async () => {
...
export const setRedisValue = (redis: Redis) => (key: string, value: string, ttl?: number): TaskEither<AppError, boolean> =>
  tryCatch(
    async () => {
      if (ttl) {
...
export const incrementRedisValue = (redis: Redis) => (key: string): TaskEither<AppError, number> =>
  tryCatch(
    () => redis.incr(key),
    (error) => createAppError({
      code: 'INTERNAL_SERVER_ERROR',
...
export const setRedisExpiry = (redis: Redis) => (key: string, ttl: number): TaskEither<AppError, boolean> =>
  tryCatch(
    async () => {
...
export const getRedisTimeToLive = (redis: Redis) => (key: string): TaskEither<AppError, number> =>
  tryCatch(
    () => redis.pttl(key),
    (error) => createAppError({
      code: 'INTERNAL_SERVER_ERROR',
...
export const deleteRedisValue = (redis: Redis) => (key: string): TaskEither<AppError, boolean> =>
  tryCatch(
    async () => {
      await redis.del(key);
      return true;
