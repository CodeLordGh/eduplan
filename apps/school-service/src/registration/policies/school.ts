...
export const createSchoolRegistrationPolicy = (action: ResourceAction): AccessPolicy =>
  createPolicy('school', action, {
    anyOf: {
...
export const createSchoolOwnerPolicy = (schoolId: string): AccessPolicy =>
  createPolicy('school', 'UPDATE', {
    school: {
      mustBeOwner: true,
    ...
    custom: [
      {
        evaluator: (attributes: UserAttributes, _conditions: PolicyConditions) =>
          hasValidSubscription(attributes, schoolId),
        errorMessage: 'Active subscription required',
      },
    ],
  });

export const createVerificationPolicy = (schoolId: string): AccessPolicy =>
  createPolicy('school', 'UPDATE', {
    anyOf: {
    ...
    custom: [
      {
        evaluator: (attributes: UserAttributes, _conditions: PolicyConditions) =>
          !attributes.schoolRoles[schoolId],
