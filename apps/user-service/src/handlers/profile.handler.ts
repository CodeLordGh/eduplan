...
export const getProfile = ({ prisma }: Dependencies) => async (
...
export const upsertProfile = ({ prisma, publishEvent }: Dependencies) => async (
...
// Update profile
export const updateProfile = ({ prisma, publishEvent }: Dependencies) => async (
  request: FastifyRequest<{
    Params: { userId: string };
    Body: unknown;
  }>,
...
// Delete profile
export const deleteProfile = ({ prisma, publishEvent }: Dependencies) => async (
  request: FastifyRequest<{ Params: { userId: string } }>,
  reply: FastifyReply
): Promise<void> => {
  const { userId } = request.params;
...
export const listProfiles = ({ prisma }: Dependencies) => async (
  request: FastifyRequest<{
    Querystring: {
      page?: number;
      limit?: number;
